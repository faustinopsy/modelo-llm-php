...
class NextWordPredictor
{
    ...
    public function __construct(string $ngramFile)
    {
        ...
        $this->featureSelector = new SelectKBest(2500, new ANOVAFValue());
    }
     ...
     * @param int  $minNgramFrequency Valor mínimo para a frequência do n-gram (se $onlyFrequent for true).
     *
    ...
    public function prepareTrainingData(bool $onlyFrequent = false, int $minNgramFrequency = 50): array
    {
        ...
        foreach ($data as $ngram => $freq) {
            ...
            $words = array_map('trim', explode(' ', $ngram));

            if (count($words) >= 2) {
                if (count($words) >= 3) {
                    $context = trim(implode(' ', array_slice($words, 0, 2)));
                    $nextWord = trim($words[2]);
                } else { // exatamente 2 tokens
                    $context = trim($words[0]);
                    $nextWord = trim($words[1]);
                }
                ...
                echo "N-Gram inválido (menos de 2 palavras): '{$ngram}'\n";
            }
        }

        $minClassFrequency = 29;
    ...
    public function train(bool $onlyFrequent = false, int $minNgramFrequency = 29): void
    {
